...
def main():
    """
    Process NICER background data.  Output will be written in current working
    directory.
    ...
    Keyword Arguments:
        emin: Minimum energy to include (keV, default=0.5)
        emax: Maximum energy to include (kev, default=8.0)
        filtpolar: FIlter polar horn regions from data
        cormin: Set minimum cutoff rigidity (COR_SAX) for nimaketime filtering
                (typical value = 4)
        dark: Apply SUNSHINE=0 filter to get only data in Earth shadow
        mask: Mask these IDS
        obsid: Use this as OBSID for directory and filename
