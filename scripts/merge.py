...
def main():
    ...
    * merged spectrum PHA produced by extrator
    ...
    Parameters:
        infile: list of event files or text file with list
        outfile: root of the output filenames
        outdir: Name of output directory

    Keyword Arguments:
        par: Par file to use for phases (in nicerql.py)
        ephem: Ephem to use with photonphase
        orb: text file containing the paths to all orbits files
        cut: perform count rate cut at the end
        lcbinsize: Lightcurve bin size (sec, default=4.0)
        filterbinsize: Bin size for Count rate filtering (sec, default=16.0)
        clobber: replace existing merged files
    """
