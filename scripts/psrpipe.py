...
def main():
    ...
    * Select events according to:
    ...
    Keyword Arguments:
        ...
        mask: Mask these IDS
        filtpolar: Turn on filtering polar horn regions from data
        cormin: Set minimum cuttoff regidity (COR_SAX) for nimaketime filtering
                (Default is no COR filtering, typical value is 4)
        kpmax: Set maximum KP value for nimaketime filtering (Default is no KP
               filtering, typical value is 5)
        minfpm: Set minimum of FPMs active for nimaketime filtering (Default is
                38)
        maxovershoot: Select data where overshoot rate is below this limit
